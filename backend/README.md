...
Be aware that my custom system logging solution requires special consideration when run on the AWS Lambda service.  When implemented on AWS Lambda you must be aware that the AWS Lambda service keeps the container for previously executed Lambda functions around for a short period of time, just in case the same Lambda function is called again.  So you must take care to reset/initialize all global objects to avoid picking up data from a prior execution of the script.

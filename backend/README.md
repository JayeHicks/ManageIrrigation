...
Be aware that my custom system logging solution requires special consideration when run on the AWS Lambda service.  The AWS Lambda service keeps containers from previously executed Lambda functions around for a short period of time in hopes that it can reuse a warm container if the same Lambda function is called again.  For this reason you must take care to always initialize / reset global variables to avoid the potential of picking up data from a previous execution of the Lambda function.
